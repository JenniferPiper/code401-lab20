...
function pVerifyPassword(password) {
  ...
  return _bcrypt2.default.campare(password, this.passwordHash).then(function (result) {
    if (!result) {
...
function pCreateToken() {
  ...
  return this.save().then(function (account) {
    return _jsonwebtoken2.default.sign({ tokenSeed: account.tokenSeed }, process.env.TEST_SECRET);
  });
}
